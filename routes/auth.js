...
router.post('/login', async (req, res) => {
  try {
    ...
    if (!passwordValido) {
      ...
      return res.status(401).json({ error: `Contraseña incorrecta. Intentos: ${intentos}/3` });
    }

    if (usuario.two_factor_enabled && !twoFactorCode) {
      return res.status(200).json({ require2FA: true, message: 'Ingresa código 2FA: 123456' });
    }
...
router.post('/recuperacion', async (req, res) => {
  try {
    ...
    const usuario = await db.queryOne('SELECT id FROM usuarios WHERE email = ?', [email]);
    if (!usuario) {
